...
#------------------------------------------------------------------
@tf.function
def training_step(ep, nstep, images, labels):
    with tf.GradientTape() as tape:
        ...
        accuracy = hvd.allreduce(accuracy, average=True)
    ...
    if (nstep == 0 and ep == 0):
        hvd.broadcast_variables(probs.variables, root_rank=0)
...
for ep in range(args.epochs):
    ...
    for batch, (images, labels) in enumerate(dataset.take(nstep)):
        loss_value, acc = training_step(ep, nstep, images, labels)
